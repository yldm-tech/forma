...
write_rustfs_env_file() {
  ...
  upsert_dotenv_var "FORMA_RUSTFS_ADMIN_USER" "$rustfs_admin_user" "$env_file"
  upsert_dotenv_var "FORMA_RUSTFS_ADMIN_PASSWORD" "$rustfs_admin_password" "$env_file"
  upsert_dotenv_var "FORMA_RUSTFS_SERVICE_USER" "$rustfs_service_user" "$env_file"
  upsert_dotenv_var "FORMA_RUSTFS_SERVICE_PASSWORD" "$rustfs_service_password" "$env_file"
  upsert_dotenv_var "FORMA_RUSTFS_BUCKET_NAME" "$rustfs_bucket_name" "$env_file"
  upsert_dotenv_var "FORMA_RUSTFS_POLICY_NAME" "$rustfs_policy_name" "$env_file"
  upsert_dotenv_var "FORMA_RUSTFS_REGION" "us-east-1" "$env_file"
}

forma_docker_command=(docker)

configure_forma_docker_command() {
  ...
  if command -v sudo >/dev/null 2>&1 && sudo docker info >/dev/null 2>&1; then
    forma_docker_command=(sudo docker)
    return
  fi
...
run_forma_docker_compose() {
  "${forma_docker_command[@]}" compose "$@"
}
...
read_rendered_compose_password() {
  ...
  rendered_config=$(
    ...
    run_forma_docker_compose "${compose_args[@]}" config --format json 2>/dev/null
  ) || return 1
...
EOF
  ...
  append_if_missing "FORMA_AUTHZED_V6_MIGRATION_ACKNOWLEDGED" "true"
...
add_forma_traefik_labels() {
  ...
  local forma_domain_name="$2"
  local forma_hsts_enabled="$3"
  local forma_https_setup="$4"
  local tmp_file="${compose_file}.tmp"

  if ! awk -v domain_name="$forma_domain_name" -v hsts_enabled="$forma_hsts_enabled" -v https_setup="$forma_https_setup" '
BEGIN { in_forma = 0; inserted = 0 }
/^  forma:$/ { in_forma = 1 }
in_forma && /^  [A-Za-z0-9_-]+:/ && !/^  forma:$/ { in_forma = 0 }
{
    if (in_forma && !inserted && $0 ~ /^    environment:$/) {
        print "    labels:"
        print "      - \"traefik.enable=true\""
        print "      - \"traefik.http.routers.forma.rule=Host(`" domain_name "`)\""
        print "      - \"traefik.http.routers.forma.entrypoints=websecure\""
        print "      - \"traefik.http.routers.forma.tls=true\""
        if (https_setup == "y") {
...
' "$compose_file" >"$tmp_file"; then
    rm -f "$tmp_file"
    echo "❌ Failed to add Traefik labels to the forma service."
    return 1
  fi
...
install_forma() {
  # Friendly welcome
  echo "🧱 Welcome to the Forma Setup Script"
  echo ""
  echo "🛸 Fasten your seatbelts! We're setting up your Forma environment on your $ubuntu_version server."
  echo ""
  ...
  if ! configure_forma_docker_command; then
  ...
  if ! run_forma_docker_compose version >/dev/null 2>&1; then
  ...
  echo "🎉 Hooray! Docker is all set and ready to go. You're now ready to run your Forma instance!"

  mkdir -p forma && cd forma
  echo "📁 Created Forma Quickstart directory at ./forma."
  ...
  if [[ $https_setup == "n" ]]; then
    echo "You have chosen not to set up HTTPS certificate for your domain. Please make sure to set up HTTPS on your own. You can refer to the Forma documentation(https://forma.ylam.ai/docs/self-hosting/custom-ssl) for more information."
...
EOT
  ...
  if [[ $configure_uploads == "y" ]]; then
    ...
    if [[ $use_external_s3 == "y" ]]; then
      ...
      rustfs_admin_user="forma-$(openssl rand -hex 4)"
      rustfs_admin_password=$(openssl rand -base64 20)
      rustfs_service_user="forma-service-$(openssl rand -hex 4)"
      rustfs_service_password=$(openssl rand -base64 20)
      rustfs_bucket_name="forma-uploads"
      rustfs_policy_name="forma-policy"
  ...
  echo "📥 Downloading docker-compose.yml from Forma GitHub repository..."
  curl -fsSL -o docker-compose.yml https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/docker-compose.yml
  ...
  if [[ $use_external_s3 == "y" ]]; then
    ...
    sed -i 's|# S3_ACCESS_KEY:|S3_ACCESS_KEY: "${FORMA_RUSTFS_SERVICE_USER}"|' docker-compose.yml
    sed -i 's|# S3_SECRET_KEY:|S3_SECRET_KEY: "${FORMA_RUSTFS_SERVICE_PASSWORD}"|' docker-compose.yml
    sed -i 's|# S3_REGION:|S3_REGION: "${FORMA_RUSTFS_REGION}"|' docker-compose.yml
    sed -i 's|# S3_BUCKET_NAME:|S3_BUCKET_NAME: "${FORMA_RUSTFS_BUCKET_NAME}"|' docker-compose.yml
    if [[ $https_setup == "y" ]]; then
  ...
  # Step 1: Add Traefik labels to forma service
...
EOF
  ...
  rustfs:
    ...
    command: /data
    environment:
      RUSTFS_ACCESS_KEY: "\${FORMA_RUSTFS_ADMIN_USER}"
      RUSTFS_SECRET_KEY: "\${FORMA_RUSTFS_ADMIN_PASSWORD}"
      RUSTFS_ADDRESS: ":9000"
    volumes:
      - rustfs-data:/data
    labels:
...
EOF
  ...
  rustfs-init:
    ...
    depends_on:
      - rustfs
    environment:
      RUSTFS_ADMIN_USER: "\${FORMA_RUSTFS_ADMIN_USER}"
      RUSTFS_ADMIN_PASSWORD: "\${FORMA_RUSTFS_ADMIN_PASSWORD}"
      RUSTFS_SERVICE_USER: "\${FORMA_RUSTFS_SERVICE_USER}"
      RUSTFS_SERVICE_PASSWORD: "\${FORMA_RUSTFS_SERVICE_PASSWORD}"
      RUSTFS_BUCKET_NAME: "\${FORMA_RUSTFS_BUCKET_NAME}"
      RUSTFS_POLICY_NAME: "\${FORMA_RUSTFS_POLICY_NAME}"
...
docker compose wait forma-migrate
...
echo "🔗 To edit more variables and deeper config, go to the forma/docker-compose.yml, edit the file, and restart the container!"
...
if [[ $rustfs_storage == "y" ]]; then
    ...
    echo "   • Generated credentials stored in ./forma/.env (permissions set to 600)"
    echo ""
fi

echo "🎉 All done! Please setup your Forma instance by visiting your domain at https://$domain_name. You can check the status of Forma & Traefik with 'cd forma && sudo docker compose ps.'"
...
uninstall_forma() {
  echo "🗑️ Preparing to Uninstalling Forma..."
  read -p "Are you sure you want to uninstall Forma? This will delete all the data associated with it! (yes/no): " uninstall_confirmation
  ...
  if [[ $uninstall_confirmation == "yes" ]]; then
    ...
    cd ..
    sudo rm -rf forma
    echo "🛑 Forma uninstalled successfully!"
  else
    echo "❌ Uninstalling Forma has been cancelled."
  fi
}
...
stop_forma() {
  ...
  cd forma
  sudo docker compose down
  echo "🎉 Forma instance stopped successfully!"
}
...
update_forma() {
  ...
  if ! grep -Eq '^  authzed-ops:$' docker-compose.yml || ! grep -Eq '^  spicedb:$' docker-compose.yml; then
    ...
    echo "https://forma.ylam.ai/docs/self-hosting/advanced/authzed-operations#upgrade-an-existing-installation-to-v6"
    exit 1
  fi

  if ! grep -Eq '^FORMA_AUTHZED_V6_MIGRATION_ACKNOWLEDGED=true$' .env; then
    ...
    echo "After its final check is clean, set FORMA_AUTHZED_V6_MIGRATION_ACKNOWLEDGED=true in .env and retry."
    echo "https://forma.ylam.ai/docs/self-hosting/advanced/authzed-operations#upgrade-an-existing-installation-to-v6"
    exit 1
  fi
  ...
  sudo docker compose run --rm forma-migrate
  ...
  echo "🎉 Forma updated successfully!"
  echo "🎉 Check the status of Forma & Traefik with 'cd forma && sudo docker compose logs.'"
}

restart_forma() {
  echo "🔄 Restarting Forma..."
  cd forma
  sudo docker compose restart
  echo "🎉 Forma restarted successfully!"
}

get_logs() {
  echo "📃 Getting Forma logs..."
  cd forma
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  uninstall)
    ...
    ;;
  *)
    echo "🚀 Executing default step of installing Forma"
    install_forma
    ;;
  esac
fi
