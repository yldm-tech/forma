...
<Info>
  Forma v5 self-hosting expects Hub to be part of the runtime. The chart handles that by default. Use the
...
Forma v5 does not support `hub.enabled=false`. Keep the default `hub.enabled=true` behavior in place.
...
Cube is part of the baseline Forma v5 stack and is bundled with the chart by default
...
| `hub.enabled`                    | Must stay `true` in Forma v5                              |
