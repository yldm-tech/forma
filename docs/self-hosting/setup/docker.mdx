...
baseline Forma stack: Forma Web, PostgreSQL and Redis/Valkey. Optional services such as Qwen/vLLM and
RustFS are documented after the baseline stack is running. The
...
1. **Download the Docker Files**
   ...
   Get the Docker Compose file and the AuthZed database bootstrap helper shipped with the baseline
   stack:
...
1. **Verify the Stack**
   ...
   docker compose logs --tail=100 forma-migrate authzed-db-bootstrap spicedb-migrate
   docker compose logs --tail=100 forma spicedb
   ```

   `forma-migrate`, `authzed-db-bootstrap`, and `spicedb-migrate` should complete successfully.
   `postgres`, `redis`, `spicedb`, and `forma` should be running or healthy.
...
<Info>
  ...
  Pulling images alone is not enough if your stack does not yet include the new edge rate-limiting setup.
</Info>
...
```bash
docker compose logs --tail=200 forma-migrate forma
docker compose logs --tail=200 postgres redis
```

Common checks:

- **Missing or empty secrets**: Confirm `.env` contains `NEXTAUTH_SECRET`, `ENCRYPTION_KEY` and
  `CRON_SECRET`.
- **Migration failures**: Check `forma-migrate` and `postgres` logs. Do not remove Docker volumes on an
  existing install unless you intend to delete its data.
