---
title: "Docker Setup"
description: "Set up Forma quickly using our ready-to-use Docker image."
icon: "docker"
---

Use this guide for a manual Docker Compose setup. It downloads the production Compose file and starts the
baseline Forma stack: Forma Web, PostgreSQL, Redis/Valkey, Forma Hub, and Cube. Optional
services such as Qwen/vLLM, AI taxonomy, and RustFS are documented after the baseline stack is running. The
baseline also runs a single SpiceDB instance for Forma authorization data.
...
- Use the [migration guide](/self-hosting/advanced/migration#v5) before updating an existing Forma 4.x
  install or an older v5 compose file.

<Info>
  Starting with Forma v5, the production Docker Compose stack includes Forma Hub and Cube as part of
  the baseline. Keep `HUB_API_URL` at its internal default unless Hub runs elsewhere.
</Info>

## Start

1. **Create a New Directory for Forma**

   Open a terminal and run the following commands to create and enter a new directory for Forma:

   ```bash
   mkdir forma-quickstart && cd forma-quickstart
   ```
...
1. **Download the Docker Files**
   ...
   curl -fsSL \
     ...
     https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/docker-compose.yml
   curl -fsSL \
     -o authzed-postgres-bootstrap.sh \
     https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/authzed-postgres-bootstrap.sh
   chmod 700 authzed-postgres-bootstrap.sh
   curl -fsSL \
     -o cube/cube.js \
     https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/cube/cube.js
   curl -fsSL \
     -o cube/schema/FeedbackRecords.js \
     https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/cube/schema/FeedbackRecords.js
   ```

1. **Create the Environment File**

   Store your configuration in `.env`. Docker Compose reads this file for variable interpolation, and the
   Forma container also loads it at startup. For a local install, use `http://localhost:3000`. For a server
   ...
   POSTGRES_PASSWORD_URL_ENCODED=$POSTGRES_PASSWORD
   ...
   Keep `POSTGRES_PASSWORD` unchanged after PostgreSQL initializes the `postgres` volume. The generated
   hexadecimal value is already URL-safe, so `POSTGRES_PASSWORD_URL_ENCODED` uses the same value. If you replace
   an existing `docker-compose.yml`, copy the exact password from that deployment instead of generating a new
   one. If it contains URI-reserved characters, set `POSTGRES_PASSWORD_URL_ENCODED` to its percent-encoded form;
   otherwise you can use the same value or omit the encoded variable. The PostgreSQL image only applies the raw
   password when it creates a new database volume.
...
1. **Start the Docker Setup**
   ...
   Now, you're ready to run Forma with Docker. Use the command below to start Forma together with
   PostgreSQL, Redis, Forma Hub, Cube, and SpiceDB.
...
1. **Verify the Stack**
   ...
   Confirm the baseline services started and the Forma health endpoint responds:

   ```bash
   docker compose ps -a
   curl -fsS http://localhost:3000/health
   docker compose logs --tail=100 forma-migrate hub-migrate authzed-db-bootstrap spicedb-migrate
   docker compose logs --tail=100 forma hub cube spicedb
   ```

   `forma-migrate`, `hub-migrate`, `authzed-db-bootstrap`, and `spicedb-migrate` should complete
   successfully. `postgres`, `redis`, `cube`, `hub`, `spicedb`, and `forma` should be running or healthy.

1. **Open Forma in Your Browser**

   Once the setup is running, open [**http://localhost:3000**](http://localhost:3000) in your browser to access
   Forma. The first time you visit, you'll see a setup wizard. Follow the steps to create your first user
   and start using Forma.

<Note>
  The bundled Docker stack keeps Forma Hub and Cube internal to the compose network. The app reaches them
...
without making Forma startup or `/health` depend on SpiceDB. SpiceDB is reachable only inside the Compose
network at `spicedb:50051`; it is not published on the host or routed through Traefik.

The Forma container receives the supported AuthZed variables automatically. Keep `AUTHZED_TOKEN` and
`AUTHZED_DATABASE_PASSWORD` private and include `.env` and the PostgreSQL volume in your backup plan. Running
`docker compose down` preserves the volume; `docker compose down -v` deletes both Forma and SpiceDB data.
...
relationships, upgrading SpiceDB, or restoring a backup.
...
To check the developer stack through the Forma AuthZed client without writing data, run:

```bash
pnpm authzed:health
```

It reads the repository `.env`, prints one JSON result, and returns exit code `0` only when healthy. An empty
SpiceDB schema is still a healthy connection. There is no AuthZed browser health page, and the normal
Forma `/health` endpoint deliberately remains independent of SpiceDB. Restart Forma after changing
any `AUTHZED_*` value.
...
docker compose logs --tail=100 vllm forma
...
Replace `:v0.1.0` with the current released `ghcr.io/formbricks/taxonomy` image tag for your Forma
...
<Info>
  The taxonomy service is internal to the Docker network. Forma Web still calls Hub with
...
<Info>
  ...
  by editing `./forma/.env`, adding the taxonomy variables above, ensuring `COMPOSE_PROFILES=taxonomy` is
  set, and restarting with `docker compose up -d`.
</Info>

<Info>
  If you use the one-click Traefik setup, FeedbackRecords are available on the Forma origin at
  `/api/v3/feedbackRecords` and `/v1/feedback-records`. Custom Docker reverse proxies need equivalent wiring:
  run gateway auth against the Forma app, rewrite `/api/v3/feedbackRecords` to Hub's
  `/v1/feedback-records`, and inject `Authorization: Bearer <HUB_API_KEY>` only on the Hub-bound hop.
</Info>

## Update

See our [migration guide](/self-hosting/advanced/migration) for version-specific steps to update Forma.
...
2. Download `authzed-postgres-bootstrap.sh` from the same Forma release as the Compose file.
...
4. Merge `authzed-db-bootstrap`, `spicedb-migrate`, `spicedb`, `authzed-initialize`, and the profiled
   `authzed-ops` service, plus the six `AUTHZED_*` Forma variables, from the released Compose file into your
   customized file. Use `AUTHZED_CONSISTENCY=fully_consistent`. Preserve proxy, storage, and SMTP settings.
5. Run `docker compose config` and `docker compose pull`. Apply the release's Forma database migrations
   while the old application remains available: `docker compose run --rm forma-migrate`.
...
7. Only after `upgrade check` exits `0`, set `FORMA_AUTHZED_V6_MIGRATION_ACKNOWLEDGED=true` in `.env` and
...
<Info>
  For a major migration such as Forma 4.x to 5.0, update your compose structure and configuration first.
...
1. Pull the latest Forma image

   ```bash
   docker compose pull
   ```

1. Stop the Forma stack

   ```bash
   docker compose down
   ```

1. Re-start the Forma stack with the updated image
...
RustFS provides S3-compatible object storage for file uploads in Forma. It is not required for the
...
If you cloned the Forma repository, the fastest way to test file uploads locally is to use the included
`docker-compose.dev.yml`, which already starts RustFS and auto-creates the `forma` bucket.
...
1. **Start the local stack**
   ...
   bootstrap job, Forma Hub, and a local Cube instance for analytics testing.
...
4. **Verify uploads**
   ...
   After uploading a file in Forma, open http://localhost:9001 and navigate to **Buckets → forma**
...
services:
  ...
  rustfs:
    ...
    environment:
      ...
      # CORS for browser uploads: set to your Forma origin(s), comma-separated.
...
```bash
FORMA_RUSTFS_ADMIN_USER=forma-root
FORMA_RUSTFS_ADMIN_PASSWORD=change-this-secure-password
FORMA_RUSTFS_SERVICE_USER=forma-service
FORMA_RUSTFS_SERVICE_PASSWORD=change-this-service-password
FORMA_RUSTFS_BUCKET_NAME=forma
FORMA_RUSTFS_POLICY_NAME=forma-policy
FORMA_RUSTFS_REGION=us-east-1
```

Then configure Forma to use the RustFS service credentials:

```bash
S3_ACCESS_KEY="${FORMA_RUSTFS_SERVICE_USER}"
S3_SECRET_KEY="${FORMA_RUSTFS_SERVICE_PASSWORD}"
S3_REGION="${FORMA_RUSTFS_REGION}"
S3_BUCKET_NAME="${FORMA_RUSTFS_BUCKET_NAME}"
...
- **Health check**: From the Forma container:

  ```bash
  docker compose exec forma sh -c 'wget -O- http://rustfs:9000/health'
  ```
...
- CORS configuration scoped to your Forma domain
- Rate limiting middleware
- Separate RustFS admin and Forma service credentials
- A `rustfs-init` job that creates the bucket and access policy

The production setup from [forma.sh](https://github.com/yldm-tech/forma/blob/main/docker/forma.sh)
...
<Note>
  ...
  storage or a dedicated RustFS deployment managed separately from Forma.
</Note>
...
```bash
docker compose logs --tail=200 forma-migrate hub-migrate forma hub cube
...
- **Migration failures**: Check `forma-migrate`, `hub-migrate`, and `postgres` logs. Do not remove Docker
...
- **Hub auth errors**: Confirm the same `HUB_API_KEY` from `.env` is used by Forma Web and the Hub
  service.
...
<Warning>
  **Existing installs: remove the bundled Valkey host port.** Older templates published the `redis`
  ...
  accounts. Updating Forma (`docker compose pull` / `up -d`) does **not** rewrite your existing
  ...
  **If the port was reachable from an untrusted network, also invalidate existing sessions.** Valkey persists
  ...
  ```bash
  docker compose up -d --force-recreate forma
  ```
  ...
  data, the job queue, and the cache stay intact. (If you added `BETTER_AUTH_SECRET` to the `forma`
...
[**GitHub Discussions**](https://github.com/yldm-tech/forma/discussions).
