...
description: "Configure file storage for survey images, file uploads, and Workspace assets in your self-hosted Forma instance"
icon: "upload"
---

Forma requires S3-compatible storage for file uploads. You can use external cloud storage services or the bundled RustFS option for a self-hosted solution.

## Why Configure File Uploads?

Setting up file storage enables important features in Forma, including:
...
Forma supports S3-compatible storage with two main configurations:
...
RustFS provides a self-hosted S3-compatible storage solution that runs alongside Forma. This option:

- Runs in a Docker container alongside Forma
...
When using the Forma installation script, you'll be prompted to configure file uploads:
...
Choose this option for a self-hosted S3-compatible storage that runs alongside Forma:
...
- Generate separate RustFS admin and Forma service credentials
- Create the storage bucket
- Configure SSL certificates for the files subdomain
- Configure Traefik routing for the subdomain
- Store the generated RustFS credentials in `./forma/.env` with restricted permissions
...
S3_REGION=us-east-1
S3_BUCKET_NAME=my-forma-uploads
...
Forma uses the S3 [POST Object](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectPOST.html)
operation (presigned POST) for uploads. Your object storage provider must support this operation. Providers
that do not implement POST Object are not compatible with Forma uploads. For example, Backblaze B2's
S3‑compatible API currently does not support POST Object and therefore will not work with Forma file
uploads.
...
- **Bucket**: Automatically created `forma-uploads` bucket
...
🗄️  RustFS Storage Setup Complete:
   • Access Key: forma-service-a1b2c3d4
   • Bucket: forma-uploads (✅ automatically created)
```
...
The following IAM policy grants the minimum required permissions for Forma to function correctly. This policy is also used in the bundled RustFS integration:
...
2. **Restricted Write Access**: Only your Forma instance should be able to upload files
3. **CORS Configuration**: Allow requests from your Forma domain
...
```json
{
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::123456789012:user/forma-service"
      },
      "Resource": "arn:aws:s3:::your-bucket-name/*",
      "Sid": "AllowFormaWrite"
    }
...
<Note>
  ...
  `arn:aws:iam::123456789012:user/forma-service` with the ARN of your IAM user. This policy allows public
  read access only to specific paths while restricting write access to your Forma service user.
</Note>

### S3 CORS Configuration

CORS (Cross-Origin Resource Sharing) must be configured on your S3 bucket to allow Forma to upload files using presigned POST URLs. Without proper CORS configuration, file uploads from the browser will fail.
...
<Note>
  For production environments, consider restricting `AllowedOrigins` to your specific Forma domain(s)
...
**Bundled Forma setup** (the [one-click script](/self-hosting/setup/one-click), driven by `docker/rustfs-init.sh`): CORS is configured for you. The bootstrap applies a per-bucket policy via `mc cors set`, scoped to your Forma domain — nothing else to do.

**Standalone RustFS** (managed separately from Forma): set `RUSTFS_CORS_ALLOWED_ORIGINS` on the RustFS server to the origin(s) you serve Forma from — your `WEBAPP_URL` — comma-separated for multiple values:
...
<Note>
  ...
  that excludes your Forma origin — set it to (or add) your origin. Use an explicit origin list in
...
- Ship RustFS and Forma container logs into your normal logging and alerting stack; the bundled setup
...
5. We use S3 presigned URLs for uploads. Make sure your CORS policy allows presigned URL uploads; otherwise, uploads will fail.
   Some providers (e.g., Hetzner’s object storage) [require a specific CORS configuration](https://github.com/yldm-tech/forma/discussions/6641#discussioncomment-14574048).
   If you’re using the bundled RustFS setup, this is already configured for you.
6. **The file appears in your storage but Forma still reports “upload failed.”** This is the classic CORS symptom — the browser uploaded the file successfully but could not read the response. For RustFS, set `RUSTFS_CORS_ALLOWED_ORIGINS` to include your Forma origin (see [RustFS CORS Configuration](#rustfs-cors-configuration)).
...
1. **Admin Panel**: Try uploading a Workspace logo in **Settings → Workspace → Appearance**
...
```bash
# Check Forma logs
docker compose logs forma

# Check RustFS logs (if using bundled RustFS)
docker compose logs rustfs
```

For additional help, join the conversation on [GitHub Discussions](https://github.com/yldm-tech/forma/discussions).
