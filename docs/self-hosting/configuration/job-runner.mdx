---
title: "Job Runner"
description: "Configure and operate the BullMQ background-job worker in self-hosted Forma"
icon: "gears"
---

Forma uses a shared BullMQ Job Runner for asynchronous work such as response processing, survey
scheduling, and [Workflows](/workflows/overview). Forma starts the worker inside the web
application by default, so a standard self-hosted deployment does not need a separate worker container.

<Note>
  Every Forma web process starts one worker whose default concurrency is derived from the host's cpu
  count - at most half the logical cpus, and never more than four. Each concurrent job can hold a
  database connection, and the Prisma pool is sized `2 * logical_cpus + 1`, so the default always leaves
  connections free for the request path. Raise it only after measuring your workload.
</Note>
...
These are current examples rather than a fixed list. Future Forma features can use the same Job Runner for
...
<Steps>
  <Step title="Connect Redis">
    Set `REDIS_URL` to a Redis instance reachable by every Forma web replica.
  ...
  <Step title="Keep the in-process worker enabled">
    ...
    Restart every Forma web process after changing these values.
...
Increase concurrency gradually and monitor database, Redis, and downstream service capacity. The binding
constraint is usually the Prisma connection pool, sized `2 * logical_cpus + 1` per process: a 2-core pod has
a pool of 5 and a default concurrency of 2, leaving three connections for the request path; an 8-core pod
has a pool of 17 and a default of 4. Adding replicas also adds workers, so account for both scaling
controls.
...
<Warning>
  Workflow delivery supports retries and recovery; it is not an exactly-once guarantee. Forma uses
...
When reconciliation changes runs, Forma writes `Workflow run reconciliation acted` with structured
...
The default in-process worker is the supported setup. Forma does not ship a standalone worker command or
container. The external-worker flags are only for custom deployments that provide their own consumer and
register the complete Forma application job handlers.
...
<Warning>
  ...
  worker. Only use this configuration after your custom consumer registers the complete Forma job
  handlers and is already consuming the same queue. Otherwise, background jobs will remain queued.
</Warning>

## Troubleshoot the Job Runner

Forma writes structured logs to stdout and stderr. Search the `msg` field for these exact messages:
...
| `BullMQ worker registration failed`                                                                | Worker startup failed. Forma schedules another attempt after 30 seconds.                                                                                                                                                                                                                  |
