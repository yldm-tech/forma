...
<Note>
  ...
  [here](/platform/features/integrations/notion) to set it up.
</Note>
...
* Now just copy from the screen the **Client ID** and **Client secret** for your integration & add it to your **Forma environment variables** as in the docker compose file:

  * `NOTION_OAUTH_CLIENT_ID` - OAuth Client ID

  * `NOTION_OAUTH_CLIENT_SECRET` - OAuth Client Secret

Voila! You have successfully enabled the Notion integration in your self-hosted Forma instance. Now you can follow the steps mentioned in [Notion Integration with Forma](/platform/features/integrations/notion) section to link your Notion with Forma.
