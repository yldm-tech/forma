...
description: "Configure third-party integrations on a self-hosted Forma instance."
icon: "bridge"
---

Forma connects to third-party tools such as Slack, Notion, or Google Sheets on behalf of your users. On
Forma Cloud we register and operate the OAuth apps behind those connections, so they work out of the
...
<Note>
  Using Forma Cloud? You do not need any of this. Head to the [Cloud integration
...
<Steps>
  <Step title="Know your public-facing URL">
    ...
    `https://<your-public-facing-url>` rather than `app.forma.ylam.ai`. The callback path differs per
    service, so take it from the guide you are following.
  </Step>

  <Step title="Serve Forma over HTTPS">
...
the third-party service. The last three connect from the other side — through a Forma API key or your
...
| [n8n](/self-hosting/configuration/integrations/n8n)                     | Nothing — n8n authenticates with a Forma API key  | None                                                                                   |
| [Zapier](/self-hosting/configuration/integrations/zapier)               | Nothing — Zapier authenticates with a Forma API key | None                                                                                 |
...
<CardGroup cols={2}>
  ...
  <Card title="Zapier" icon="bolt" href="/self-hosting/configuration/integrations/zapier">
    Connect Forma to thousands of apps on Zapier.
  </Card>
...
Forma Cloud. Follow the matching [Cloud guide](/platform/features/integrations/overview) from there.

<Note>
  For automations that stay inside Forma — without a third-party tool — use
...
Discussions](https://github.com/yldm-tech/forma/discussions) and we'd be glad to assist you!
