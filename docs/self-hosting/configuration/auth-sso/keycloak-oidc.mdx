---
title: "Keycloak OIDC"
description: "Configure Keycloak as an OpenID Connect (OIDC) provider for secure Single Sign-On with your self-hosted Forma instance."
icon: "key-skeleton-left-right"
---

<Note>Keycloak OIDC is part of the Forma [Enterprise Edition](/self-hosting/advanced/license)</Note>

Integrating Keycloak with your Forma instance allows users to log in using their existing Keycloak credentials via OpenID Connect. This guide walks you through setting up a Keycloak OIDC client and configuring Forma to use it, including automatic user provisioning into a default team.

### Requirements

- A running Keycloak instance with a configured realm and users.
- A self-hosted Forma instance with a valid [Enterprise license](/self-hosting/advanced/license).
- The Forma callback URI: `{WEBAPP_URL}/api/auth/oauth2/callback/openid`
...
<Warning>
  **Upgrading from v5.1 or earlier?** This path changed once, at v5.2:
  ...
  Forma' authentication library. See the [migration guide](/self-hosting/advanced/migration).
</Warning>
...
<Steps>
  <Step title="Create a new client in Keycloak">
    ...
    - Choose a **Client ID** (e.g., `forma`) and click **Next**.
  </Step>
  ...
  <Step title="Configure client authentication">
    ...
    <Note>
      Forma uses PKCE (Proof Key for Code Exchange) for added security. Modern Keycloak versions (21+) support PKCE automatically for confidential clients. No additional PKCE configuration is needed.
    </Note>
  </Step>

  <Step title="Set the redirect URI">
    - Under **Valid redirect URIs**, add your Forma callback URI:

    ```
    https://your-forma-domain.com/api/auth/oauth2/callback/openid
    ```

    Replace `your-forma-domain.com` with your actual Forma URL.
  ...
  <Step title="Copy the client secret">
    ...
    - Copy the **Client secret** value — you will need this for the Forma configuration.
  </Step>
  ...
  <Step title="Configure Forma environment variables">
    Add the following environment variables to your Forma `.env` file or `docker-compose.yml`:

    ```yml Forma Env for Keycloak OIDC
    OIDC_CLIENT_ID=forma
    ...
    | --- | --- |
    | `OIDC_CLIENT_ID` | The Client ID you chose in Keycloak (e.g., `forma`). |
    | `OIDC_CLIENT_SECRET` | The client secret from the **Credentials** tab. |
    | `OIDC_ISSUER` | Your Keycloak realm URL (see step above). |
    | `OIDC_DISPLAY_NAME` | The label shown on the Forma login button. |
    | `OIDC_SIGNING_ALGORITHM` | Optional. Keycloak and Forma both default to `RS256`. Only set this if your realm uses a different algorithm. |
  </Step>

  <Step title="Restart and test">
    - Restart your Forma instance.
...
By default, new SSO users must be invited by an organization admin before they can access Forma. To allow Keycloak users to join automatically and be assigned to a team, configure the following additional environment variables:
...
<Note>
  The Teams feature requires the **Role-Based Access Control (RBAC)** add-on on your Enterprise license. If you don't see the Teams section under **Settings → Organization**, contact [Forma support](mailto:support@forma.ylam.ai) to enable it.
</Note>
...
2. Forma creates a new user account.
...
<AccordionGroup>
  ...
  <Accordion title="Users see 'No membership found' after login">
    Forma determines the organization automatically from the `AUTH_SSO_DEFAULT_TEAM_ID` — you do not need to set a separate organization ID. If this error appears:
  ...
  <Accordion title="OIDC login button does not appear">
    ...
    - Ensure your Forma instance has a valid Enterprise license.
    - Restart the Forma container after adding the environment variables.
  </Accordion>
  ...
  <Accordion title="Authentication fails with a redirect error">
    ...
    - Check that the OIDC issuer URL is reachable from your Forma server. You can test with:
    ```bash
