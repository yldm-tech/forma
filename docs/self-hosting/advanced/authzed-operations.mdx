...
description: "Configure, monitor, back up, and repair the SpiceDB authorization dependency used by Forma."
icon: "shield-halved"
---

Forma uses [AuthZed SpiceDB](https://authzed.com/docs/spicedb) to store a relationship graph for
authorization. PostgreSQL remains the source of truth. Forma projects membership, team, workspace, and
API-key changes into SpiceDB after their PostgreSQL transaction commits.

<Note>
  Forma v6 makes SpiceDB the sole authorization decision engine and has no runtime legacy fallback.
...
| ---------------------------- | ------------------------------------------------------------------------------- |
| Forma PostgreSQL        | Authoritative organizations, memberships, teams, workspaces, and API-key scopes |
...
| Forma projection layer  | Copies committed PostgreSQL authorization changes into SpiceDB                  |
| AuthZed operator commands    | Check health and schema state, then audit or repair relationship drift          |

Forma writes projection intent in the same PostgreSQL transaction as the source mutation. BullMQ wakes the
...
The general Forma `/health` endpoint, application startup, and Kubernetes readiness and liveness probes
do not depend on SpiceDB. This prevents a SpiceDB outage from restarting unrelated Forma workloads. It
...
| `AUTHZED_SYSTEM_KEY`        | Stable Forma authorization namespace identifier                | `forma`                                        |
...
postgres → forma-migrate
spicedb + forma-migrate → authzed-initialize
```
...
`authzed-initialize` is an idempotent, one-shot service that prepares fresh installations. Forma does not
...
<Warning>
  ...
  the Forma and SpiceDB databases. Do not use `-v` during an upgrade or incident response.
</Warning>
...
The Forma chart supports two modes:

- `authzed.mode=selfHosted` creates a `SpiceDBCluster` and points Forma at its private Kubernetes Service.
- `authzed.mode=external` configures the Forma client for AuthZed Cloud or another external SpiceDB.
...
Forma namespace, set `authzed.operator.install=false`. Apply the matching SpiceDB CRDs before upgrading
...
authzed:
  bundledPostgresqlBootstrap:
    ...
    adminDatabase: forma # maintenance database to attach to
...
datastore and Forma migrations become available. Application startup and probes do not depend on this Job.
...
```bash
kubectl exec -n forma deployment/forma -- forma-authzed health
kubectl exec -n forma deployment/forma -- forma-authzed schema check
kubectl exec -n forma deployment/forma -- forma-authzed backfill
kubectl exec -n forma deployment/forma -- forma-authzed upgrade check
```
...
Forma v6 has no legacy authorization fallback. Do not deploy it over an installation whose graph has not
been proven complete. First upgrade to the bridge-compatible v5 release named in the v6 release notes, enable
durable projection, and take coordinated Forma and SpiceDB backups.
...
For one-click, set `FORMA_AUTHZED_V6_MIGRATION_ACKNOWLEDGED=true` in `.env` only after those commands pass.
...
<Steps>
  <Step title="Back up both datastores">
    Create a PostgreSQL-consistent backup of the Forma database and the dedicated `spicedb` database.
    Record the deployed Forma and SpiceDB versions, configuration, and schema digest.
  </Step>
  ...
  <Step title="Audit and reconcile relationships">
    ...
    ```bash
    forma-authzed backfill
    forma-authzed backfill --apply
    ```
  ...
  <Step title="Restart and observe">
    Restart Forma after environment changes. Recheck AuthZed health, schema status, projection metrics,
...
```bash
umask 077
docker compose exec -T postgres pg_dump -U postgres -d forma > forma.sql
...
1. Restore coordinated Forma and SpiceDB database backups from the same recovery point.
2. Restore Forma PostgreSQL, initialize an empty compatible SpiceDB datastore, apply the release-matched
...
can be repaired from PostgreSQL; a SpiceDB database restored without its matching Forma source must never
be assumed current.

AuthZed Cloud owns datastore backup and restore guarantees. You still need Forma PostgreSQL backups,
...
Every six hours Forma also runs a full applying audit without prune. It automatically repairs attributable
...
Most are Forma record IDs, but not all — `unmanaged` can surface object IDs belonging to no Forma
...
umask 077
rm -f backfill.json
forma-authzed backfill > backfill.json
```
...
```bash
forma-authzed backfill
forma-authzed backfill --organization-id=<organization-id>
forma-authzed backfill --workspace-id=<workspace-id>
```
...
```bash
forma-authzed backfill --apply
forma-authzed backfill --apply --organization-id=<organization-id>
forma-authzed backfill --apply --workspace-id=<workspace-id>
```
...
```bash
forma-authzed backfill --apply --after-organization-id=<last-organization-id>
```
...
```bash
forma-authzed backfill --apply --organization-id=<failed-organization-id>
```
...
```bash
forma-authzed backfill --apply --prune --confirm-prune \
  --scope=all \
...
repair. A complete prune requires a SpiceDB datastore dedicated to one Forma deployment because object
...
| `unmanaged`         | A relationship outside Forma' managed vocabulary exists. Investigate its writer; the tool never deletes it.                            |
...
| `authzed_internal`                                                                       | Unexpected client or runtime failure                  | Check sanitized Forma and SpiceDB logs and the deployed version                        |
| Schema status `drifted`                                                                  | Connected schema differs from this Forma release | Back up first, review the diff counts and digest, then perform a guarded schema apply       |
...
Forma AuthZed logs use `component="authzed"`, stable `errorCode` values, and bounded fields such as
...
Enable the existing Forma Prometheus or OTLP metrics exporter. The release-matched direct-authority
signals are:

| Metric                                               | What it measures                                                            |
| ---------------------------------------------------- | --------------------------------------------------------------------------- |
| `forma_authzed_projection_total`                | Projected, failed, and disabled projection outcomes                         |
| `forma_authzed_projection_duration_seconds`     | Request-path projection latency                                             |
| `forma_authzed_request_failures_total`          | AuthZed requests that exhausted their retry budget                          |
| `forma_authzed_request_retries_total`           | Retry attempts caused by transient failures                                 |
| `forma_authzed_authorization_decisions_total`   | Authoritative allow, deny, and operational-error outcomes                    |
| `forma_authzed_authorization_decision_duration_seconds` | End-to-end authoritative authorization latency                      |
| `forma_authzed_authorization_checks_per_request` | Central authorization operations per authenticated request                  |
| `forma_authzed_projection_outbox_delivery_total` | Durable outbox events delivered or failed                                   |
| `forma_authzed_projection_outbox_delivery_duration_seconds` | Durable delivery batch latency                                    |
| `forma_authzed_projection_revocation_delivery_duration_seconds` | Commit-to-SpiceDB revocation propagation time                     |
| `forma_authzed_projection_outbox_status`        | Point-in-time pending, dead-letter, warning, and critical counts             |
| `forma_authzed_projection_outbox_oldest_pending_age_seconds` | Age of the oldest pending event                                  |
| `forma_authzed_reconciliation_audit_total`      | Scheduled audit outcomes                                                     |
| `forma_authzed_reconciliation_drift_total`      | Attributable drift and failures observed by scheduled audits                 |
| `forma_authzed_reconciliation_repair_total`     | Repaired and failed attributable relationship repair results                 |

Starting PromQL checks:

```promql
# Projection failures introduce possible drift.
sum(rate(forma_authzed_projection_total{status="failed"}[5m]))

# Terminal unavailability errors.
sum(rate(forma_authzed_request_failures_total{code="authzed_unavailable"}[5m]))

# Projection latency p95.
histogram_quantile(
  0.95,
  sum by (le) (rate(forma_authzed_projection_duration_seconds_bucket[5m]))
)
...
/
sum(rate(forma_authzed_authorization_decisions_total[5m]))

# Direct-authority latency p95 by surface.
histogram_quantile(
  0.95,
  sum by (le, surface) (rate(forma_authzed_authorization_decision_duration_seconds_bucket[5m]))
)

# Revocations that missed the warning or critical delivery thresholds.
forma_authzed_projection_outbox_status{state=~"revocation_warning|revocation_critical"}

# Dead letters and failed scheduled repairs both block cutover.
forma_authzed_projection_outbox_status{state="dead_lettered"}
or
sum(rate(forma_authzed_reconciliation_repair_total{status="failed"}[5m]))
```
...
1. Confirm Forma `/health` independently.
2. Run `forma-authzed health`.
3. Check SpiceDB pods or containers and the last datastore migration.
4. Check Forma projection failures, AuthZed terminal errors, retries, and latency.
