---
title: "Rate Limiting"
description: "Current request rate limits in Forma"
icon: "timer"
---

Forma applies request rate limits to protect against abuse and keep API usage fair.

Starting with Forma v5, rate limiting is split across two layers:
...
<Warning>
  ...
  protected by Envoy Gateway. If you self-host Forma without Envoy or an equivalent edge rate limiter,
...
- Organization ID (for follow-up email dispatch and invitation budgets)

When a limit is exceeded, the API returns `429 Too Many Requests`.

## v5 Migration Note

Before upgrading to Forma v5:
...
These limits are expected to be enforced at the gateway layer in Forma v5 and later:
...
These are the limits that still run inside the Forma application server:
...
| `actions.inviteMember`        | 50 recipients | 24 hours  | Organization ID               | Single, bulk, onboarding, and resend invite actions                                                                                                       |
...
Invite limits are recipient-counted, so a bulk request for 25 people consumes 25 units. Self-hosters can
change the limit with `INVITE_RATE_LIMIT_PER_24_HOURS`; Forma Cloud organizations with the
`bulk-invite` entitlement receive a limit of 500 recipients.
