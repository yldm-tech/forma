---
title: "Migration"
description: "Forma Self-hosted version migration"
icon: "arrow-right"
---

## v6

Forma v6 replaces the legacy authorization evaluator with AuthZed SpiceDB. PostgreSQL remains the source
...
1. Back up the Forma and SpiceDB PostgreSQL databases, secrets, deployment configuration, image versions,
...
4. Run `forma-authzed upgrade prepare`, then require `forma-authzed upgrade check` to exit `0`.
5. For one-click or Compose, set `FORMA_AUTHZED_V6_MIGRATION_ACKNOWLEDGED=true`. For Helm, set
...
<Note>
  Both variables below drive SSO just-in-time provisioning, which is part of the Forma
  [Enterprise Edition](/self-hosting/advanced/license). On a community instance they have no effect, so
  there is nothing to rename.
</Note>

- **DEFAULT_ORGANIZATION_ID** has been renamed to **AUTH_SSO_DEFAULT_ORGANIZATION_ID**, matching the
  `AUTH_SSO_DEFAULT_TEAM_ID` naming pattern. Rename the variable in your configuration; the value is
  unchanged. Support for this variable was missing between v3.0 and v5.x, so it takes effect again on v6.

- **DEFAULT_ORGANIZATION_ROLE** has been renamed to **AUTH_SSO_DEFAULT_ORGANIZATION_ROLE**.
...
<Warning>
  ...
  for `forma-migrate` — it has no `depends_on` on that service and skips startup migrations itself —
...
<Tabs>
  <Tab title="One-Click">
    ...
    file. It creates `forma/docker-compose.yml.before-valkey-8.1.9`, validates the updated Compose file,
    and restores the backup if validation fails. Custom Valkey images are left unchanged.

    ```bash
    curl -fsSL https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/forma.sh \
      -o forma.sh
    chmod +x forma.sh
    ./forma.sh update
    ```
  ...
  <Tab title="Helm">
    ...
    ```bash
    helm upgrade forma forma/forma \
      --reuse-values \
...
real tokens against it fails every Microsoft sign-in with `?error=unable_to_get_user_info`. Forma
...
keeps restricting sign-in to work/school accounts, and an unset value behaves as `common`. Forma
...
Forma pins the callback URL instead, so it no longer follows the authentication library's internal
routing. The URLs above are the ones introduced in v5.2 and they are now a stable part of the Forma
...
<Note>
  ...
  providers accept several redirect URIs, and Forma simply will not use it.
</Note>
...
that caches a Forma MCP access token and reuses it across sessions will start receiving rejections
...
Forma v5.3 is a **drop-in upgrade for most self-hosted instances**. There are no breaking API,
...
- `TRUSTED_PROXY_HOP_COUNT` — optional, **defaults to `1`**. Set it to the number of trusted proxies /
  load balancers in front of Forma (e.g. an ingress plus a CDN = `2`). An explicit `0` trusts no
...
Forma v5.2 migrates authentication from NextAuth to **Better Auth**. For most self-hosted
...
Forma v5 changes the self-hosted runtime contract. If you are upgrading an existing Forma 4.x
deployment, review this section before starting the new version.

### What Changes In v5

- **Forma Hub is now mandatory** for self-hosted Forma v5 deployments.
...
<Warning>
  Forma v5 removes application-level rate limiting for several routes that are now expected to be
...
Before you restart your instance on Forma v5:
...
Forma v5 expects Hub to be part of the self-hosted stack.

- `HUB_API_KEY` is required
- `HUB_API_URL` must point to the Hub service the Forma app can reach
- `HUB_DATABASE_URL` is optional; when unset, Hub can share the same PostgreSQL database as Forma
...
<Note>
  ...
  [Forma Hub repository](https://github.com/yldm-tech/hub). For a normal Forma v5 upgrade, use that
  repository as reference material only; the canonical migration steps stay in these Forma docs.
</Note>

#### Edge Rate Limiting

Forma v5 splits rate limiting across two layers:
...
Cube is part of the baseline Forma v5 stack.

- the Docker, one-click, and Helm deployments all bundle the `cube` service by default
- the Forma app requires `CUBEJS_API_URL` and `CUBEJS_API_SECRET`; the install/dev-setup scripts
...
<Tabs>
  <Tab title="One-Click">
    From the `forma` directory created by the installer:

    ```bash
    cd forma
    docker compose exec postgres pg_dump -Fc -U postgres -d forma > forma_pre_v5_$(date +%Y%m%d_%H%M%S).dump
    ```
  ...
  <Tab title="Kubernetes">
    ...
    ```bash
    kubectl exec -n forma forma-postgresql-0 -- pg_dump -Fc -U forma -d forma > forma_pre_v5_$(date +%Y%m%d_%H%M%S).dump
    ```
...
<Tabs>
  <Tab title="One-Click">
    The one-click `./forma.sh update` command only pulls new images. It does **not** rewrite your existing
    `forma/docker-compose.yml`, so you must merge the v5 stack changes before the first v5 restart.

    ```bash
    curl -fsSL -o forma/docker-compose.v5.yml https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/docker-compose.yml
    ```

    Then compare `forma/docker-compose.v5.yml` with your existing `forma/docker-compose.yml` and merge
    the v5 additions:

    - set `POSTGRES_PASSWORD` in `forma/.env` to the exact password used by the existing PostgreSQL
      volume; do not generate a new value during the upgrade. If it contains URI-reserved characters, also set
      `POSTGRES_PASSWORD_URL_ENCODED` to its percent-encoded form
    ...
    - include the bundled `forma-migrate`, `hub-migrate`, and `hub` services
    - sync `forma/cube/cube.js` and `forma/cube/schema/FeedbackRecords.js` from the current
      release and ensure `forma/.env` contains `CUBEJS_API_SECRET` (Cube is part of the baseline stack
      in v5)
  ...
  <Tab title="Docker Compose">
    ...
    ```bash
    curl -fsSL -o docker-compose.v5.yml https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/docker-compose.yml
    ```

    At minimum, confirm:

    - `POSTGRES_PASSWORD` contains the exact password used by the existing PostgreSQL volume; replacing the
      Compose file does not rotate the database password. If it contains URI-reserved characters, also set
      `POSTGRES_PASSWORD_URL_ENCODED` to its percent-encoded form
    ...
    - the compose stack includes `forma-migrate`, `hub-migrate`, and `hub`
  ...
  <Tab title="Kubernetes">
    Upgrade using the current Forma chart:
...
- confirm the Forma app starts and `GET /health` returns successfully
- confirm the Hub service is healthy and reachable from the Forma app
...
- **Missing `HUB_API_KEY`**: Forma cannot authenticate to Hub, and Hub itself may fail to start
...
- **Missing `CUBEJS_API_SECRET`** (or unreachable Cube endpoint): the Forma app fails env validation
...
- revert your image tags or Helm release values to the last known-good Forma 4.x version
...
<Info>
  Forma v5 also includes the Environment ID to Workspace ID transition for SDK setup. If you still rely on a
  legacy Environment ID, read the [Migrate to Workspace ID guide](/surveys/website-app-surveys/workspace-id-migration).
</Info>

## v4.7

Forma v4.7 introduces **typed contact attributes** with native `number` and `date` data types. This enables comparison-based segment filters (e.g. "signup date before 2025-01-01") that were previously not possible with string-only attribute values.

### What Happens Automatically

When Forma v4.7 starts for the first time, the data migration will:
...
<Tabs>
  <Tab title="Docker">
    Before running these steps, navigate to the `forma` directory where your `docker-compose.yml` file is located.

    ```bash
    docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v4.7_$(date +%Y%m%d_%H%M%S).dump
    ```
  ...
  <Tab title="Kubernetes">
    ...
    ```bash
    kubectl exec -n forma forma-postgresql-0 -- pg_dump -Fc -U forma -d forma > forma_pre_v4.7_$(date +%Y%m%d_%H%M%S).dump
    ```

    <Info>
      If your PostgreSQL pod has a different name, run `kubectl get pods -n forma` to find it.
    </Info>
...
After Forma starts, check the logs to see whether the value backfill was completed or skipped:

<Tabs>
  <Tab title="Docker">
    ```bash
    docker compose logs forma | grep -i "backfill"
    ```
  </Tab>
  <Tab title="Kubernetes">
    ```bash
    # Check the application pod logs
    kubectl logs -n forma -l app.kubernetes.io/name=forma --tail=200 | grep -i "backfill"
    ```

    If the Helm migration Job ran, you can also inspect its logs:

    ```bash
    kubectl logs -n forma job/forma-migration
    ```
...
If the migration skipped the value backfill, run the standalone backfill script inside the running Forma container:

<Tabs>
  <Tab title="Docker">
    ```bash
    docker exec forma node packages/database/dist/scripts/backfill-attribute-values.js
    ```

    <Info>Replace `forma` with your actual container name if it differs. Use `docker ps` to find it.</Info>

  </Tab>
  <Tab title="Kubernetes">
    ```bash
    kubectl exec -n forma deploy/forma -- node packages/database/dist/scripts/backfill-attribute-values.js
    ```

    <Info>
      If your Forma deployment has a different name, run `kubectl get deploy -n forma` to find it.
    </Info>
...
- **Safe to run while Forma is live** -- it does not lock the entire table or wrap work in a long transaction
...
- Access your Forma instance at the same URL as before
...
Forma 4 introduces additional requirements for self-hosting setups and makes a dedicated Redis cache as well as S3-compatible file storage mandatory.

</Warning>

Forma 4.0 is a **major milestone** that sets up the technical foundation for future iterations and feature improvements. This release focuses on modernizing core infrastructure components to improve reliability, scalability, and enable advanced features going forward.

### What's New in Forma 4.0
...
These improvements in Forma 4.0 also make some infrastructure requirements mandatory going forward:
...
Additional migration steps are needed if you are using a self-hosted Forma setup that uses either local file storage (not S3-compatible file storage) or doesn't already use a Redis cache.
...
curl -fsSL -o migrate-to-v4.sh \
  https://raw.githubusercontent.com/yldm-tech/forma/stable/docker/migrate-to-v4.sh
...
- Pulls the latest Forma image and updates your instance

### Manual Setup

If you use a different setup to host your Forma instance, you need to make sure to make the necessary adjustments to run Forma 4.0.

#### Redis

Forma 4.0 requires a Redis instance to work properly. Please add a Redis instance to your Docker setup, your K8s infrastructure, or however you are hosting Forma at the moment. Forma works with the latest versions of Redis as well as Valkey.

You need to configure the `REDIS_URL` environment variable and point it to your Redis instance.

#### S3-compatible storage

To use file storage (e.g., file upload questions, image choice questions, custom survey backgrounds, etc.), you need to have S3-compatible file storage set up and connected to Forma.

Forma supports multiple storage providers (among many other S3-compatible storages):
...
Please make sure to set up a storage bucket with one of these solutions and then link it to Forma using the following environment variables:

```
  S3_ACCESS_KEY: your-access-key
  S3_SECRET_KEY: your-secret-key
  S3_REGION: us-east-1
  S3_BUCKET_NAME: forma-uploads
...
**Critical Step**: Create a complete database backup before proceeding. Forma 4.0 will automatically remove unused database tables and fields during startup.

```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v4.0_$(date +%Y%m%d_%H%M%S).dump
```
...
# Start with Forma 4.0
docker compose up -d
```

**3. Automatic Database Migration**

When you start Forma 4.0 for the first time, it will **automatically**:
...
<Info>
  With Forma 3.0, we introduced an automatic data migration system. If you're using a version older than
...
This guide is for users who are **self-hosting Forma** using the **one-click setup**. If you have a different setup, you may need to adjust the commands accordingly.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v3.3_$(date +%Y%m%d_%H%M%S).dump
```

<Info>
  If you run into “**No such container**”, use `docker ps` to find your container name,
  e.g. `forma_postgres_1`.
</Info>

If you prefer storing the backup as an `*.sql` file remove the `-Fc` (custom format) option. In case of a restore scenario you will need to use `psql` then with an empty `forma` database.

1. Pull the latest version of Forma:

```bash
docker compose pull
```

1. Stop the running Forma instance & remove the related containers:

```bash
docker compose down
```

1. Restarting the containers with the latest version of Forma:

```bash
docker compose up -d
```

When you start the latest version of **Forma**, it will **automatically detect and apply any required data migrations** during startup. You don’t need to run any manual migration steps or download separate migration images.

- Access your updated instance

Once the containers are running, simply go to the **same URL** as before to access your updated **Forma** instance.

That’s it! This new process ensures your **Forma** setup stays up to date with the latest schema changes as soon as you run the updated container.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v3.2_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v3.1_$(date +%Y%m%d_%H%M%S).dump
```
...
With **Forma 3.0**, we're making changes to ensure long-term sustainability while still supporting open source. While the **Community Edition** has gained [new features](https://forma.ylam.ai/blog/forma-3-0), some [advanced capabilities](https://forma.ylam.ai/docs/self-hosting/license) are now part of the **Enterprise Edition**.

⚠️ **No Downgrade Option:** If you upgrade to **3.0** and run the data migration, **you cannot revert to 2.7.2**. If you rely on **SSO, user identification, or cluster support**, either **stay on version 2.7.x** or reach out to us on [**GitHub Discussions**](https://github.com/yldm-tech/forma/discussions) **for a custom quote**.

</Warning>

This major release brings a better approach to **data migrations**.

- If you're using **Forma v2.7 or later**, you can upgrade **directly** to **v3.x** and future versions.

- If you're on a version **older than v2.7**, you **must upgrade step-by-step** (e.g. **2.4 → 2.5 → 2.6 → 2.7**) before moving to v3.x.

### Steps to Migrate

This guide is for users **self-hosting** Forma with the **one-click setup**. If you're using a different setup, you might adjust the commands.

- &#x20;Navigate to the Forma Directory
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v3.0_$(date +%Y%m%d_%H%M%S).dump
...
<Info>
  ...
  to restore it later, use `psql` with an empty Forma database.
</Info>

- Pull the latest version of Forma:

```bash
docker compose pull
```

- Stop the running Forma instance & remove the related containers:

```bash
docker compose down
```

- Restarting the containers with the latest version of Forma:

```bash
docker compose up -d
```

When you start the latest version of Forma, it will automatically detect and run any necessary data migrations during startup. There is no need to run any manual migration steps or pull any separate migration images.

- Access your updated instance

That’s it! Once the containers are up and running, simply navigate to the same URL as before to access your fully migrated Forma instance.

## v2.7

<Info>
  AI Features & Database Update: This release lays the groundwork for upcoming AI features, now in private beta. Forma requires the `pgvector` extension in PostgreSQL.

One-Click Setup Users: Use the `pgvector/pgvector:pg15` image instead of `postgres:15-alpine` to ensure compatibility.

</Info>

Forma v2.7 includes all the features and improvements developed by the community during Hacktoberfest 2024. Additionally, this release introduces an advanced team-based access control system, which is available in the Forma Enterprise Edition.
...
This guide is for users who are self-hosting Forma using our one-click setup. If you are using a different setup, you might adjust the commands accordingly.

To run all these steps, please navigate to the Forma folder where your `docker-compose.yml` file is located.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.7_$(date +%Y%m%d_%H%M%S).dump
```
...
- Stop the running Forma instance and remove the related containers:
...
```bash
docker pull ghcr.io/yldm-tech/data-migrations:latest && \
docker run --rm \
  --network=forma_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/forma?schema=public" \
  -e UPGRADE_TO_VERSION="v2.7" \
  ghcr.io/yldm-tech/data-migrations:v2.7.0
...
- That's it! Once the migration is finished, you can **access your Forma instance** at the same URL as before.

## v2.6

Forma v2.6 introduces advanced logic jumps for surveys, allowing more complex branching logic, including variables, conditions, and more. This release also includes numerous bug fixes, significant performance improvements for website and app surveys, and enhanced stability.
...
This guide is for users who are self-hosting Forma using our one-click setup. If you're using a different setup, you might need to adjust the commands accordingly.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.6_$(date +%Y%m%d_%H%M%S).dump
```
...
<Info>
  ...
  restore scenario, you will need to use `psql` then with an empty `forma` database.
</Info>
...
docker run --rm \
  ...
  ghcr.io/yldm-tech/data-migrations:v2.6.0
```
...
- Once the migration is complete, **you can access your Forma instance** at the same URL as before.

## v2.5

Forma v2.5 lets you visualise responses in a data table format. This release also brings a few bug fixes and performance improvements.
...
This guide is for users self-hosting Forma with our one-click setup. If you're using a different setup, you may need to adjust the commands.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.5_$(date +%Y%m%d_%H%M%S).dump
```
...
docker run --rm \
  ...
  ghcr.io/yldm-tech/data-migrations:v2.5.3
```
...
Forma v2.4 lets you create multiple endings for your surveys and choose which one users see based on logic jumps. This version also includes bug fixes and performance improvements.
...
<Info>
  ...
  triggers, but they will no longer be stored on the server to improve Forma' overall performance.
</Info>

### Steps to Migrate

This guide is for users who are self-hosting Forma with our one-click setup. If you're using a different setup, you may need to adjust the commands accordingly.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.4_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/yldm-tech/data-migrations:v2.4.3 && \
docker run --rm \
  --network=forma_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/forma?schema=public" \
  -e UPGRADE_TO_VERSION="v2.4" \
  ghcr.io/yldm-tech/data-migrations:v2.4.3
```
...
- That's it! Once the migration is done, you can access your Forma instance at the same URL as before.
...
Forma v2.3 introduces new colour options for rating questions, enhanced multi-language support for Chinese (Simplified & Traditional), and includes several bug fixes and performance improvements.
...
This guide is for users self-hosting Forma with our one-click setup. If you're using a different setup, adjust the commands as needed.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.3_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/yldm-tech/data-migrations:v2.3.0 && \
docker run --rm \
  --network=forma_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/forma?schema=public" \
  -e UPGRADE_TO_VERSION="v2.3" \
  ghcr.io/yldm-tech/data-migrations:v2.3.0
...
The feature to create short URLs in Forma is now deprecated. Existing short URLs will continue to work for now, but it is recommended to use long URLs instead. Support for short URL redirects will be removed in a future release.

## v2.2

Forma v2.2 introduces XM research presets, providing a brand-new product onboarding experience. Our goal is to make user research "obviously easy" across different industries, starting with Software-as-a-Service and E-Commerce.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.2_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/yldm-tech/data-migrations:v2.2 && \
docker run --rm \
  --network=forma_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/forma?schema=public" \
  -e UPGRADE_TO_VERSION="v2.2" \
  ghcr.io/yldm-tech/data-migrations:v2.2
```
...
Forma v2.1 introduces more options for creating No-Code Actions and improves the self-hosting experience with a new onboarding process for fresh instances.
...
This guide is for users self-hosting Forma using our one-click setup. If you are using a different setup, you may need to adjust the commands accordingly.

To follow these steps, please navigate to the Forma folder where your `docker-compose.yml` file is located.
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.1_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/yldm-tech/data-migrations:v2.1.0 && \
docker run --rm \
  --network=forma_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/forma?schema=public" \
  -e UPGRADE_TO_VERSION="v2.1" \
  ghcr.io/yldm-tech/data-migrations:v2.1.0
```

Once the migration script runs successfully, your data will be migrated to the latest schema. The script is designed to run safely multiple times, so you don’t need to worry if it’s executed more than once. After completing the migration, you can continue to access your Forma instance at the same URL as before.

- That's it! Once the migration is complete, you can **now access your Forma instance** at the same URL as before.
...
Forma v2.0 introduces significant features such as **Multi-Language Surveys** and **Advanced Styling for Surveys**, aimed at improving the flexibility and appearance of your surveys. Additionally, the release includes several optimisations, bug fixes, and smaller improvements to ensure a smoother user experience.

If you are upgrading to Forma v2.0, follow this guide to ensure a smooth migration of your existing Forma instance without encountering issues or build errors.

<Info>
  This upgrade requires a **data migration**. Please make sure to back up your database before proceeding.
  Follow the steps below to upgrade your Forma instance to v2.0.
</Info>

<Info>
  If you've used the Forma Enterprise Edition with a free beta license key, your instance will be
  downgraded to the Community Edition 2.0. You can find all license details on the [license
  page](https://forma.ylam.ai/docs/self-hosting/license).
</Info>
...
<Warning>
  ...
  forma/forma:latest` to `image:ghcr.io/yldm-tech/forma:latest` in
...
```bash
docker exec forma-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v2.0_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/yldm-tech/data-migrations:v2.0.3 && \
docker run --rm \
  --network=forma_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/forma?schema=public" \
  -e UPGRADE_TO_VERSION="v2.0" \
  ghcr.io/yldm-tech/data-migrations:v2.0.3
```
...
```ts
import forma from "@forma/js/website";
...
```ts
import forma from "@forma/js/app";
...
Forma v1.6 introduces key features like Advanced Targeting, Segmentation, on-the-fly survey triggers, and stability improvements. This update also changes some environment variables. Follow this guide to upgrade your Forma instance to v1.6 smoothly.

<Info>
  This upgrade requires a data migration. Ensure you back up your database before proceeding. Follow the steps
  below to upgrade your Forma instance to v1.6.
</Info>

### Steps to Migrate

This guide is for users self-hosting Forma with our one-click setup. If you're using a different setup, adjust the commands accordingly.

Navigate to the Forma folder containing your `docker-compose.yml` file to run the steps.

- **Backup your Database**: This is a crucial step. Please make sure to backup your database before proceeding with the upgrade. You can use the following command to backup your database:

```bash
docker exec forma-quickstart-postgres-1 pg_dump -Fc -U postgres -d forma > forma_pre_v1.6_$(date +%Y%m%d_%H%M%S).dump
```
...
- Restarting the containers will automatically pull the latest version of Forma:
...
docker run --rm \
  ...
  ghcr.io/yldm-tech/data-migrations:v1.6.1

```

The above command will migrate your data to the latest schema. This is a crucial step to migrate your existing data to the new structure. Only if the script runs successful, changes are made to the database. The script can safely run multiple times.

1. That's it! Once the migration is complete, you can **now access your Forma instance** at the same URL as before.

#### Restoring the database after a failed upgrade

```bash
docker exec -i forma-quickstart-postgres-1 pg_restore --clean -U postgres -v -d forma < forma_pre_v1.6_<timestamp_of_your_dump_file>.dump
```

<Info>
  Replace the path to `forma_pre_v1.6_<timestamp_of_your_dump_file>.dump` with the exact path to your `.dump` file.
</Info>
...
If you're using the `@forma/js` package, update it to version \~1.6.5 to access the latest features and improvements.

<Info>
  Currently, the package needs to be pinned to `~1.6.5`, see [this
  issue](https://github.com/yldm-tech/forma/issues/2273).
</Info>

### Upgrade and pin the client package

```bash
npm install @forma/js@~1.6.5
```
...
Forma v1.2 introduces new features for Link Surveys and enhances security. However, there are some breaking changes related to environment variables. This guide will help you upgrade your Forma instance to v1.2 smoothly.
...
Forma v1.1 introduces new features and improvements but includes breaking changes with environment variables. This guide will help you upgrade your Forma instance to v1.1 without data loss.
...
| `NEXT_PUBLIC_IS_FORMA_CLOUD`         | `IS_FORMA_CLOUD`         |
...
x-environment: &environment
  environment:
    # The url of your Forma instance used in the admin panel
    WEBAPP_URL:

    # Required for next-auth. Should be the same as WEBAPP_URL
    NEXTAUTH_URL:

    # PostgreSQL DB for Forma to connect to
    DATABASE_URL: "postgresql://postgres:postgres@postgres:5432/forma?schema=public"
...
](https://forma.ylam.ai/docs/developer-docs/rest-api)
