...
<Warning>
  This fork's database history is a single init baseline: upstream's 202 migrations were squashed when
  Unify Feedback and Dashboards were removed. The upgrade paths below describe upstream releases and do
  not apply to a database created from this fork.
</Warning>
...
v5.4 adds no new environment variables. It does ship
...
5. Smoke test: sign in (including 2FA), then create and schedule a survey.
