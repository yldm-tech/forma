...
description:
  "Copy surveys and their responses from one Forma instance to another with the management API."
icon: "right-left"
---

This page is about moving **selected surveys and their response data** between two running Forma
instances — a staging box to production, a self-hosted instance to Forma Cloud, one company's
...
<Note>
  **Moving a whole instance? Do not use this page.** If you control both machines and want everything
  ...
  ```bash
  pg_dump "$OLD_DB_URL" -Fc -f forma.dump
  pg_restore -d "$NEW_DB_URL" --no-owner forma.dump
  ```

  Restore into an instance running the **same Forma version** as the source, then upgrade it
...
<Steps>
  <Step title="Create an API key on each instance">
    ...
    [Generate API Key](/api-reference/generate-key). Read access is enough on the source; the target
...
#!/usr/bin/env bash
# Copy one survey and all of its responses from one Forma instance to another.
set -euo pipefail

SOURCE_URL="${SOURCE_URL:?e.g. https://old.forma.example.com}"
...
TARGET_URL="${TARGET_URL:?e.g. https://new.forma.example.com}"
...
```bash
SOURCE_URL=https://old.forma.example.com \
SOURCE_API_KEY=fbk_xxx \
SOURCE_SURVEY_ID=cmrsxxwej0009tm06aq5i4lx0 \
TARGET_URL=https://new.forma.example.com \
...
link:      https://new.forma.example.com/s/cmt8wewit0008s706rsqaqtf1
```
...
```bash fb-export.sh
#!/usr/bin/env bash
# Export every survey, response and action class of one Forma workspace.
set -euo pipefail
...
```bash fb-import.sh
#!/usr/bin/env bash
# Import a ./fb-export dump into a target Forma workspace. Idempotent it is not:
...
- **Update your SDK setup** if you are moving app surveys: `workspaceId` and `appUrl` in
  `Forma.setup()` both still point at the old instance.
