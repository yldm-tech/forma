---
title: "Workflows"
description: "Automate response-driven tasks with triggers, filters, and actions."
icon: "https://d3gk2c5xim1je2.cloudfront.net/lucide/v1.16.0/workflow.svg"
sidebarTitle: "Workflows"
---

Workflows automate tasks in response to events in Forma. You choose a trigger, narrow down which events qualify with optional filters, and run an action. Today that action is sending an email.

Read the full guide: [Workflows overview](/workflows/overview).
