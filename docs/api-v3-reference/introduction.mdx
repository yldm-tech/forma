...
description: "The next generation of the Forma management API, including the Feedback Records API"
icon: "code-compare"
hidden: true
---
...
- **Kubernetes / Helm** — off by default. Set `envoy.enabled: true` and
  `envoy.forma.routes.feedbackRecords: true`. Enabling the route also installs its authorization policy,
  so the two cannot drift apart.
- **One-click Docker** — the installer writes the Traefik routes for you, so it works out of the box.
- **Forma Cloud** — already enabled.
...
reports it as `dataset_id`, or ask your Forma contact.
...
curl "https://app.forma.ylam.ai/api/v3/feedbackRecords?tenant_id=<datasetId>&field_type=text&sentiment=negative&sentiment=very_negative&limit=20" \
  -H "x-api-key: $FORMA_API_KEY"
```
