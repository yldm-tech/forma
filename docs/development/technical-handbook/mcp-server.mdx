...
| `surveys:write` | `create_survey`, `patch_survey`, `delete_survey`, `archive_survey`, `restore_survey` |
...
| `archive_survey` | `write` or `manage` |
| `restore_survey` | `write` or `manage` |
| `list_workflows` | `read` |
| `get_workflow` | `read` |
...
### archive_survey / restore_survey

Archive is the reversible counterpart of `delete_survey`: it soft-deletes the survey, hides it from the
default list, and `restore_survey` brings it back. Both take only a `surveyId` and return the lifecycle
acknowledgement the v3 routes return (`id`, `status`, `archivedAt`) rather than the full survey document.
...
An archived survey is excluded from `list_surveys` unless the status filter asks for it:
`filter.status.in: ["archived"]`. That value is a pseudo-status accepted only by the list filter — it is
translated into an `archivedAt` filter, not matched against a real survey status. On its own it returns
*only* archived surveys; combine it with real statuses to get both. `patch_survey` refuses an archived
survey with a 422 telling the caller to restore it first.
...
| `archive_survey`  | `POST /api/v3/surveys/{surveyId}/archive` |
| `restore_survey`  | `POST /api/v3/surveys/{surveyId}/restore` |
