...
Charts in Forma are not built on survey responses. They are built on **Feedback Records**, which is why a
...
- **Measures that know what a score means.** Because the record is typed, Forma can offer `NPS: Score`,
    ...
    **Add feedback source**.
    ...
    <Frame>
      <img
        ...
        alt="Add feedback source dialog offering Forma surveys, CSV import, API ingestion and MCP server"
      />
    </Frame>

    Pick **Forma surveys**, then **Select questions**, choose your survey and tick the questions you want to
    report on. Forma detects each question's type for you, so an NPS question becomes an `nps` record without
    any mapping work.
