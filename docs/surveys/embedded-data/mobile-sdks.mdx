...
<Note>
  ...
  those come from the survey renderer, which is loaded from your Forma instance at display time, so an
...
```javascript
import { setEmbeddedData, clearEmbeddedData } from "@forma/react-native";
...
Forma.clearEmbeddedData("plan")  // one key
Forma.clearEmbeddedData()        // everything
```
...
<Warning>
  **On iOS, `nil` removes a key — there is no "leave this alone" value.** Swift has no `undefined`, so this
  ...
  Forma.setEmbeddedData(["plan": user.plan.map(EmbeddedDataValue.string)])
  ```
...
The [reserved fields](/surveys/embedded-data/reserved-fields) work in a WebView with **no extra wiring**: the survey renderer is loaded from your Forma instance at display time, so an already-installed app picks this up without a rebuild or an SDK upgrade.
