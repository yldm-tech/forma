...
description: "The system fields Forma captures on every response without you declaring anything: page, campaign, device, timing and request context."
icon: "list-check"
---
...
- **Server** — derived from the request or from the stored response when it reaches Forma. Available on the response, in filters and in exports, but not offered to a running survey, because mid-survey the answer does not exist yet.
...
| `country`         | `string`  | Server          | Country from the CDN request header. Forma runs no GeoIP lookup          |
...
Link surveys and website & app surveys render through the same component, so both capture the full browser set. What those fields *describe* differs: on a link survey `url`, `pagePath`, `pageReferrer` and `utm*` are about the Forma-hosted survey page and how the respondent reached it; on an app survey they are about the host page the survey was triggered on.
...
<Note>
  ...
  That is browser behavior rather than a Forma decision, so compare `viewportWidth` against a breakpoint
