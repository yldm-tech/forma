...
description: "Attach context from your app to the next Forma response, without tying it to a trigger. The JavaScript SDK reference for setEmbeddedData and clearEmbeddedData."
icon: "code"
---

In an app survey the integrator usually does not know which action will open a survey: your app fires a dozen of them and Forma decides. Passing context on `track()` therefore means repeating the same values on every possible call.
...
```js
forma.setEmbeddedData({ pageType: "product", plan: "pro" });
```
...
```js
forma.setEmbeddedData({ plan: "pro", pageType: "product" });
forma.setEmbeddedData({ pageType: "checkout" });
...
```js
forma.setEmbeddedData({ pageType: null });      // removes pageType
forma.setEmbeddedData({ pageType: undefined }); // no-op — pageType keeps its value
```
...
```js
forma.clearEmbeddedData("pageType"); // remove one key
forma.clearEmbeddedData();           // clear the whole bag
```
...
<Steps>
  ...
  <Step title="Cleared on an identity switch">
    Calling `forma.logout()`, or `forma.setUserId()` with a **different** id, clears the bag: one
...
function onRouteChange(route) {
  ...
  forma.setEmbeddedData({ pageType: route.pageType ?? null });
...
forma.on("forma_setup_successful", () => {
  forma.setEmbeddedData({ plan: currentUser.plan });
});
```
...
The bag is invisible: it is memory, so there is nothing in devtools storage, and the API has no getter. Add `?formaDebug=true` to your URL and every write prints:
...
The older `forma.track("action", { hiddenFields: { … } })` form still works and still wins. An explicit per-trigger value beats the ambient bag for the same field, whatever the casing on either side, so migrating one call site at a time is safe.

<Note>
  Two ways to load the SDK, two ways to pick this up. The script served by your Forma instance
  (`/js/forma.umd.cjs`) carries it as soon as the instance is updated, because that bundle is served
