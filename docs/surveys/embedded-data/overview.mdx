...
description: "Everything on a response that is not an answer to a question: values you pass in, values the survey calculates, and the context Forma captures on its own."
icon: "database"
---
...
| **Reserved** | Forma, automatically, on every response | not authored at all       | `string`, `number`, `boolean`, `date` |
...
- **Reserved fields** are typed by Forma, and several of them are genuinely numeric: `viewportWidth < 768` is a numeric comparison in logic, not a comparison of digit strings.
...
2. an explicit `forma.track("action", { hiddenFields: { plan: "pro" } })` value;
