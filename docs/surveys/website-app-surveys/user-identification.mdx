...
In Forma, understanding the difference between identified and unidentified users is crucial for effective survey segmentation and targeted feedback collection.
...
| Collect **user information** in Forma               | ❌                 | ✅               |
...
Identified users are those for whom specific information has been set, notably the userId. This identification allows for more precise targeting of surveys and a deeper understanding of the feedback provided. When enabled, all information specified by you and all actions are sent to Forma.

This method is recommended for applications where users are required to log in and will often return.

### Setting User ID

To enable user identification, call the `setUserId` function of Forma and pass the user id. The user will show up in the Forma dashboard. Use a unique string, like a database ID or a unique email address. You can also anonymize the identifier, as long as it is unique for each user.

```javascript
forma.setUserId("<user-id>");
```
...
forma.setEmail("hola@forma.ylam.ai");
```
...
forma.setEmail("hola@forma.ylam.ai");
forma.setAttributes({ email: "hola@forma.ylam.ai" });
```

Order matters: call `setUserId` before `setEmail`. Attributes are only stored for identified users, so if no user ID has been set yet, Forma drops the email and logs an error to the console:

```text
🧱 Forma - <timestamp> [ERROR] - Forma can't set attributes without a userId! Please set a userId first with the setUserId function
```

<Note>
  **Note**: an email address can belong to only one contact per workspace. If another contact already
  has the address you pass, Forma keeps that contact's email and silently skips the update. The SDK
...
forma.setAttribute("Plan", "free");
```

The `setAttribute` function works like this:

```javascript Setting Custom Attributes
forma.setAttribute("attribute_key", "attribute_value");
```
...
forma.setLanguage("de"); // ISO identifier or Alias set when creating language
```

### Logging Out Users

When a user logs out of your webpage, also log them out of Forma to prevent activity from being linked to the wrong user. Use the logout function:
