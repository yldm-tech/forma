...
description:
  "URL Data Prefilling for Link Surveys in Forma"
icon: "fill"
---

Data prefilling via the URL allows you to increase completion rate by prefilling data you already have in a different system. Forma allows you to prefill multiple questions in a survey and skip prefilled questions.
...
- Have data in a different system (e.g. your database) and want to add it to the user profile in Forma
- Want to embed a survey in an email and increase completion by prefilling the choice selected in the email

## Quick Example

```sh Example URL
https://app.forma.ylam.ai/s/clin3dxja02k8l80hpwmx4bjy?question_id_1=answer_1&question_id_2=answer_2&skipPrefilled=true
```

## How it works

1. To prefill survey questions, add query parameters to the survey URL using the format `questionId=answer`.
2. The answer must match the question’s expected type to pass [validation](/surveys/link-surveys/data-prefilling#validation). For Single Select and Multi Select questions, `answer` can be either the option label text or the option ID.
...
app.forma.ylam.ai/s/clin3dxj?question_id_1=answer_1&skipPrefilled=true
```

### Prefilling multiple values

Forma lets you prefill as many values as you want. Combine multiple values in the URL using `&`, for example: `name=Bernadette&age=18`. The order of query parameters doesn’t matter, so you can move questions around or add new ones without worrying about the parameter order.

### Where do I find my question Id?

Open a question in the Survey Editor and click **Show Question settings** at the foot of its card. The `questionId` is the first field there. You can change it to any string you like **before you publish a survey**; after publishing it is fixed, because the responses already collected refer to it.

![The question Id is located at the bottom of each question card in the survey editor.](/images/surveys/link-surveys/data-prefilling/question-id.webp)
...
```sh Rating Question
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?rating_question_id=5
```
...
```sh NPS Questions
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?nps_question_id=10
```
...
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?single_select_question_id=Very%20disappointed
```
...
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?multi_select_question_id=Sun%2CPalms%2CBeach
```

### Open Text Question

Adds 'I love Forma' as the answer to the open text question:

```sh Open Text Question
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?openText_question_id=I%20love%20Forma
```
...
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?consent_question_id=accepted
```
...
https://app.forma.ylam.ai/s/clin3yxja52k8l80hpwmx4bjy?pictureSelection_question_id=1%2C2%2C3
```
...
https://app.forma.ylam.ai/s/cmoh0584taxoxu501jyvk8tzn?tuk2i9ktui1lee0mfos82czb=e2r697fou23n7ba4vmt7you6
```
...
3. Click **Show Question settings** at the foot of the question card
...
https://app.forma.ylam.ai/s/surveyId?questionId=option-abc123
```

**Multi Select:**

```sh Multi Select with Option IDs
https://app.forma.ylam.ai/s/surveyId?questionId=option-1,option-2,option-3
```
...
```sh Mixed Approach
https://app.forma.ylam.ai/s/surveyId?questionId=option-abc,Some%20Label,option-xyz
```
