...
<Note>
  **Looking for responses tied to a specific person? Use
  ...
  Single-use links are anonymous. Forma limits each link to one response but does not know who
  ...
  and email. Forma also blocks a second response from the same contact on its own, so you get
...
```
https://app.forma.ylam.ai/s/<surveyId>?suId=<single-use-id>&suToken=<signature>
```

When someone opens the link, Forma:

<Steps>
  <Step title="Validates the suId">
    The `suId` must be one Forma issued for this survey. If it is missing, malformed, or
  ...
  <Step title="Looks for an existing response">
    Forma checks whether a response with this `suId` already exists for the survey.
  </Step>
...
| Who creates the ID | Forma (UI or API) | You |
...
<Warning>
  Forma does not store the generated links. Save the CSV — links cannot be re-downloaded, and
...
```bash
curl -X GET \
  'https://app.forma.ylam.ai/api/v1/management/surveys/<surveyId>/singleUseIds?limit=100' \
...
{
  "data": [
    "https://app.forma.ylam.ai/s/clzqym2l3...?suId=ohkj3qx6d0wdo9r3nfpm3bgj&suToken=9f2b...",
    "https://app.forma.ylam.ai/s/clzqym2l3...?suId=ci6la2lzj63pvsp6x3eqbhff&suToken=41ce..."
  ]
}
```
...
Forma re-validates the URL and rejects the response if the two do not match.
...
- **Since Forma 5.0**, unencrypted single-use links must carry a valid `suToken`. Links
