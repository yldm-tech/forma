...
```
https://forma.ylam.ai/s/clin34bjy?screen=pricing
```

Multiple Hidden Fields:

```
https://forma.ylam.ai/s/clin34bjy?screen=landing_page&job=Founder
```
...
```js
forma.setEmbeddedData({ my_field: "value" });
```
...
```js
forma.track("action_name", { hiddenFields: { my_field: "value" } });
```
