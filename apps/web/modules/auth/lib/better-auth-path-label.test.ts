...
describe("createAuthPathLabeller — the security invariant (ENG-2259)", () => {
  test("never emits the token from /reset-password/:token", () => {
    const result = label(`https://app.forma.ylam.ai/api/auth/reset-password/${RESET_TOKEN}`);
  ...
  test("never emits the parameter of ANY declared parameterized route", () => {
    ...
    for (const pattern of parameterized) {
      ...
      expect(label(`https://app.forma.ylam.ai/api/auth${concrete}`)).not.toContain(RESET_TOKEN);
    }
  });

  test("drops the query string, which carries OAuth codes and verification tokens", () => {
    const result = label(`https://app.forma.ylam.ai/api/auth/oauth2/token?code=${RESET_TOKEN}&state=abc123`);
...
describe("createAuthPathLabeller — labelling rules", () => {
  test("emits a parameter-free declared path whole", () => {
    expect(label("https://app.forma.ylam.ai/api/auth/sign-in/email")).toBe("/sign-in/email");
    expect(label("https://app.forma.ylam.ai/api/auth/get-session")).toBe("/get-session");
  });
  ...
  test("keeps the oauth2 endpoints distinct, which is what the MCP-OAuth lead needs", () => {
    const labels = ["/oauth2/token", "/oauth2/userinfo", "/oauth2/authorize"].map((path) =>
      label(`https://app.forma.ylam.ai/api/auth${path}`)
    );
  ...
  test("falls back to the first segment for an undeclared deeper path", () => {
    expect(label("https://app.forma.ylam.ai/api/auth/sign-in/passkey")).toBe("/sign-in/*");
  });

  test("labels an unrecognized first segment unknown, bounding tag cardinality", () => {
    expect(label("https://app.forma.ylam.ai/api/auth/not-an-endpoint")).toBe(UNKNOWN_AUTH_PATH_LABEL);
    expect(label("https://app.forma.ylam.ai/api/auth/%2e%2e/etc/passwd")).toBe(UNKNOWN_AUTH_PATH_LABEL);
  });

  test("resolves the path when the app is served from a Next.js basePath subpath", () => {
    expect(label("https://example.com/forma/api/auth/sign-in/email")).toBe("/sign-in/email");
  });

  test("labels a URL outside the auth base path, or an unparseable one, unknown", () => {
    expect(label("https://app.forma.ylam.ai/api/v1/surveys")).toBe(UNKNOWN_AUTH_PATH_LABEL);
  ...
  test("every emitted label is rooted or the unknown marker, so one Sentry facet stays filterable", () => {
    ...
    ].map((path) => label(`https://app.forma.ylam.ai${path}`));
  ...
  test("a pathological run of slashes stays linear (no catastrophic backtracking)", () => {
    ...
    // regressed cost.
    const pathological = `https://app.forma.ylam.ai/api/auth/${"/".repeat(100_000)}x`;
  ...
  test("trailing-slash trimming matches the regex it replaced, including the edge shapes", () => {
    ...
    expect(label("https://app.forma.ylam.ai/api/auth/get-session")).toBe("/get-session");
    expect(label("https://app.forma.ylam.ai/api/auth/get-session/")).toBe("/get-session");
    expect(label(`https://app.forma.ylam.ai/api/auth/get-session${"/".repeat(50)}`)).toBe("/get-session");
    // Path reduces to "" — the loop must stop at index 0 rather than underflow.
    expect(label("https://app.forma.ylam.ai/api/auth///")).toBe(UNKNOWN_AUTH_PATH_LABEL);
    expect(label("https://app.forma.ylam.ai/api/auth")).toBe(UNKNOWN_AUTH_PATH_LABEL);
  });
  ...
  test("a truncated label never collides with the same-named literal endpoint", () => {
    ...
    expect(label("https://app.forma.ylam.ai/api/auth/reset-password")).toBe("/reset-password");
    expect(label(`https://app.forma.ylam.ai/api/auth/reset-password/${RESET_TOKEN}`)).toBe(
      "/reset-password/*"
    );
  });
  ...
  test("ignores declared entries that are not usable paths", () => {
    ...
    expect(tolerant("https://app.forma.ylam.ai/api/auth/get-session")).toBe("/get-session");
    expect(tolerant("https://app.forma.ylam.ai/api/auth/no-leading-slash")).toBe(UNKNOWN_AUTH_PATH_LABEL);
  });
});
