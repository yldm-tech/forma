...
import { type CacheService, getCacheService } from "@forma/cache";
import { prisma } from "@forma/database";
import { IntegrationType, type Organization } from "@forma/database/prisma";
...
describe("sendTelemetryEvents", () => {
  ...
  test("should skip telemetry when TELEMETRY_ENABLED is not set", async () => {
    vi.resetModules();
    vi.doMock("@/lib/constants", () => ({
      E2E_TESTING: false,
      IS_DEVELOPMENT: false,
