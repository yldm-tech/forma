...
describe("getPublicDomain", () => {
  ...
  test("returns trimmed WEBAPP_URL when configured", async () => {
    envMock.WEBAPP_URL = " https://app.forma.ylam.ai ";

    const getPublicDomain = await loadGetPublicDomain();

    expect(getPublicDomain()).toBe("https://app.forma.ylam.ai");
  });
  ...
  test("returns PUBLIC_URL when set", async () => {
    envMock.WEBAPP_URL = "https://app.forma.ylam.ai";
    envMock.PUBLIC_URL = "https://surveys.forma.ylam.ai";

    const getPublicDomain = await loadGetPublicDomain();

    expect(getPublicDomain()).toBe("https://surveys.forma.ylam.ai");
  });
