 ...
 * `@forma/database` prisma singleton (both bind to `process.env` at import time). setupFiles run
 * before the test module's imports, so these assignments win over the `.env` loaded by `dotenv -e`.
 *
 * NOTE: this harness does NOT mock `@forma/database` (the unit harness, vitestSetup.ts, does) —
 * the whole point is to exercise the real prisma client against a real Postgres.
 */
process.env.DATABASE_URL =
  process.env.TEST_DATABASE_URL ??
  "postgresql://postgres:postgres@localhost:5432/forma_ba_test?schema=public";
