...
grep --fixed-strings 'forma-authzed health' <<<"${authzed_notes}" >/dev/null
grep --fixed-strings 'forma-authzed schema check' <<<"${authzed_notes}" >/dev/null
grep --fixed-strings 'forma-authzed upgrade prepare' <<<"${authzed_notes}" >/dev/null
grep --fixed-strings 'forma-authzed upgrade check' <<<"${authzed_notes}" >/dev/null
...
grep --fixed-strings 'name: forma-authzed-initialize' <<<"${default_install}" >/dev/null
...
if grep --fixed-strings 'name: forma-authzed-initialize' <<<"${externally_prepared_install}" >/dev/null; then
...
if authzed_disabled_error="$(helm template authzed-disabled "${CHART_DIR}" "${COMMON_ARGS[@]}" \
  ...
  printf '%s\n' "Forma v6 must refuse a chart deployment with AuthZed disabled." >&2
  exit 1
fi
grep --fixed-strings 'Forma v6 requires AuthZed' <<<"${authzed_disabled_error}" >/dev/null
...
helm template authzed-existing-operator "${CHART_DIR}" "${COMMON_ARGS[@]}" \
  ...
  --set authzed.operator.install=false \
  --set authzed.auth.existingSecret=forma-authzed \
  --set authzed.datastore.existingSecret=forma-authzed >/dev/null
...
existing_operator_notes="$(render_notes authzed-existing-operator \
  ...
  --set authzed.operator.install=false \
  --set authzed.auth.existingSecret=forma-authzed \
  --set authzed.datastore.existingSecret=forma-authzed)"
...
managed_postgresql_notes="$(render_notes authzed-managed-postgresql \
  ...
  --set-string postgresql.externalDatabaseUrl=postgresql://forma:notes-secret@postgres.example:5432/forma?sslmode=require \
  --set authzed.enabled=true \
  --set authzed.mode=selfHosted \
  --set authzed.operator.install=false \
  --set authzed.auth.existingSecret=forma-authzed \
...
helm template authzed-external "${CHART_DIR}" "${COMMON_ARGS[@]}" \
  ...
  --set authzed.endpoint=grpc.authzed.com:443 \
  --set authzed.insecure=false \
  --set authzed.auth.existingSecret=forma-authzed >/dev/null
...
existing_admin_bootstrap="$(render_bootstrap authzed-bootstrap-existing-admin \
  ...
  --set authzed.bundledPostgresqlBootstrap.adminDatabase=forma \
...
grep --quiet 'value: "forma"' <<<"${existing_admin_bootstrap}"
