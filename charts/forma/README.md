...
Cube is part of the baseline Forma v5 stack and is deployed by this chart by default
(`cube.enabled: true`).

- For the chart-managed Cube, the chart renders `deployment.env.CUBEJS_API_URL` automatically as
  `http://forma-cube:4000` when using the default release name.
...
When the Forma migration job is enabled, Hub waits for the `forma-migration` Job to complete before its own goose/river init migrations run. This keeps fresh shared-database installs from creating Hub tables before Prisma has initialized the Forma schema.
...
When deployed with Argo CD, chart-managed Secrets, ExternalSecrets, and bundled PostgreSQL render in sync wave `-2`, and the Forma and Hub migration hooks run in sync wave `-1`. This lets app and Hub secrets exist and PostgreSQL become healthy before migration jobs start.
...
hub:
  ...
  extraVolumeMounts:
    - name: google-cloud-credentials
      mountPath: /var/run/secrets/forma/google
      readOnly: true

  env:
    GOOGLE_APPLICATION_CREDENTIALS: /var/run/secrets/forma/google/credentials.json
...
```yaml
TAXONOMY_SERVICE_URL: http://forma-taxonomy:8000
...
```sh
kubectl exec -n forma deploy/forma-taxonomy -- \
...
hub:
  env:
    ...
    OTEL_RESOURCE_ATTRIBUTES: deployment.environment=production,service.namespace=forma
...
taxonomy:
  env:
    ...
    OTEL_EXPORTER_OTLP_PROTOCOL: http/protobuf
    OTEL_EXPORTER_OTLP_ENDPOINT: http://signoz-otel-collector.signoz.svc.cluster.local:4318
    OTEL_SERVICE_NAME: forma-taxonomy
    OTEL_RESOURCE_ATTRIBUTES: deployment.environment=production,service.namespace=forma
```
...
| hub.migration.waitForFormaMigration.enabled                   | bool   | `true`                                                                      |                                                           |
| hub.migration.waitForFormaMigration.intervalSeconds           | int    | `5`                                                                         |                                                           |
| hub.migration.waitForFormaMigration.maxAttempts               | int    | `180`                                                                       |                                                           |
| hub.migration.waitForFormaMigration.missingJobMaxAttempts     | int    | `12`                                                                        | Consecutive missing Job reads before using DB markers.    |
